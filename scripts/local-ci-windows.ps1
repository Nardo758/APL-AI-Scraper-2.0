...
function Show-ReportSummary {
    Write-Host "`nTEST REPORT SUMMARY`n" -ForegroundColor Magenta
    $currentDir = (Get-Location).Path
    if (Test-Path "coverage/lcov.info") { Write-Host "Coverage Report: file://$currentDir/coverage/lcov-report/index.html" }
    if (Test-Path "test-results/junit") { Write-Host "Test Results: file://$currentDir/test-results/junit/" }
    if (Test-Path "test-results/eslint/results.xml") { Write-Host "ESLint: file://$currentDir/test-results/eslint/results.xml" }
}

# Main
$mode = if ($args.Count -gt 0) { $args[0].ToLower() } else { 'stubbed' }
switch ($mode) {
    'stubbed' { Invoke-LocalStubbedTests; Show-ReportSummary }
    'integration' { Invoke-LocalIntegrationTests; Show-ReportSummary }
    'all' { Invoke-LocalStubbedTests; Invoke-LocalIntegrationTests; Show-ReportSummary }
    'report' { Show-ReportSummary }
    default { Write-Host "Usage: .\scripts\local-ci-windows.ps1 {stubbed|integration|all|report}" -ForegroundColor Yellow; exit 1 }
}

Write-Host "`nLocal CI simulation complete" -ForegroundColor Green
